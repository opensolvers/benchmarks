...
void force_calc(CellStructure &cell_structure, double time_step, double kT) {
...
#if defined(ELECTROSTATICS) && defined(P3M)
  CoulombP3M const *p3m_actor = nullptr;
  if (electrostatics_actor && !elc_kernel && !dipoles_kernel) {
    if (auto const *sp = boost::get<std::shared_ptr<CoulombP3M>>(
  ...
  if (p3m_actor) {
    short_range_loop(
        ...
        [p3m_actor](Particle &p1, Particle &p2, Distance const &d) {
          add_non_bonded_pair_force_p3m(p1, p2, d.vec21, sqrt(d.dist2),
                                        d.dist2, *p3m_actor);
#ifdef COLLISION_DETECTION
