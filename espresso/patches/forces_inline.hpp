...
#ifdef P3M
/** Specialized hot path: WCA/LJ + direct CoulombP3M::pair_force (no std::function /
 *  ForceKernelRef indirection). Used when the electrostatics actor is plain P3M. */
inline void add_non_bonded_pair_force_p3m(Particle &p1, Particle &p2,
                                          Utils::Vector3d const &d, double dist,
                                          double /*dist2*/,
                                          CoulombP3M const &p3m_actor) {
  IA_parameters const &ia_params = *get_ia_param(p1.type(), p2.type());
  ParticleForce pf{};
  ...
  auto const q1q2 = p1.q() * p2.q();
  if (q1q2 != 0.) {
    pf.f += p3m_actor.pair_force(q1q2, d, dist);
  }
