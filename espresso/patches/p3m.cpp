...
double CoulombP3M::long_range_kernel(bool force_flag, bool energy_flag,
  ...
  if (force_flag) {
    /* sqrt(-1)*k differentiation — pointer locals for compiler (opensolvers opt) */
    auto const * const g_force = p3m.g_force.data();
    auto const * const rs_mesh = p3m.rs_mesh.data();
    auto * const e0 = p3m.E_mesh[0].data();
    auto * const e1 = p3m.E_mesh[1].data();
    auto * const e2 = p3m.E_mesh[2].data();
    auto const li = box_geo.length_inv();
    auto const two_pi = 2. * Utils::pi();
    ...
    for (j[0] = 0; j[0] < p3m.fft.plan[3].new_mesh[0]; j[0]++) {
      for (j[1] = 0; j[1] < p3m.fft.plan[3].new_mesh[1]; j[1]++) {
        for (j[2] = 0; j[2] < p3m.fft.plan[3].new_mesh[2]; j[2]++) {
          auto const rho_re = rs_mesh[2 * ind];
          auto const rho_im = rs_mesh[2 * ind + 1];
          auto const gf = g_force[ind];
          auto const phi_re = gf * rho_re;
          auto const phi_im = gf * rho_im;

          for (int d = 0; d < 3; d++) {
            auto const d_rs = (d + p3m.ks_pnum) % 3;
            auto const k =
                two_pi * p3m.d_op[d_rs][j[d] + p3m.fft.plan[3].start[d]] * li[d_rs];
            auto * const e = (d_rs == 0) ? e0 : ((d_rs == 1) ? e1 : e2);
            e[2 * ind + 0] = -k * phi_im;
            e[2 * ind + 1] = +k * phi_re;
          }
...
void CoulombP3M::scaleby_box_l() {
  ...
  rebuild_sr_force_table();
}

void CoulombP3M::rebuild_sr_force_table() {
  auto const r_cut = p3m.params.r_cut;
  auto const alpha = p3m.params.alpha;
  if (!(r_cut > 0.) || !(alpha > 0.)) {
    m_sr_force_table.clear();
    m_sr_dr_inv = 0.;
    return;
  }
  // Dense enough that linear interp error << P3M accuracy target.
  constexpr int n_bins = 4096;
  m_sr_force_table.resize(static_cast<unsigned>(n_bins) + 1u);
  auto const dr = r_cut / static_cast<double>(n_bins);
  m_sr_dr_inv = 1.0 / dr;
  auto const two_a_sqrt_pi_i = 2.0 * alpha * Utils::sqrt_pi_i();
  // Avoid singular r=0: use tiny epsilon for first point.
  m_sr_force_table[0] = 0.;
  for (int i = 1; i <= n_bins; ++i) {
    auto const dist = dr * static_cast<double>(i);
...
#endif
    m_sr_force_table[static_cast<unsigned>(i)] = fac * prefactor;
  }
}
