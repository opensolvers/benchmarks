...
public:
  ...
  /** Real-space force factor table: stores prefactor * fac(r) on a uniform r grid.
   *  Avoids exp/erfc in the pair loop (opensolvers opt). */
  void rebuild_sr_force_table();
  ...
  Utils::Vector3d pair_force(double q1q2, Utils::Vector3d const &d,
    ...
    if (!m_sr_force_table.empty()) {
      auto const x = dist * m_sr_dr_inv;
      auto i = static_cast<int>(x);
      auto const n = static_cast<int>(m_sr_force_table.size()) - 2;
      if (i < 0) {
        i = 0;
      } else if (i > n) {
        i = n;
      }
      auto const t = x - static_cast<double>(i);
      auto const fac =
          m_sr_force_table[static_cast<unsigned>(i)] * (1.0 - t) +
          m_sr_force_table[static_cast<unsigned>(i) + 1u] * t;
      return (fac * q1q2) * d;
    }
...
private:
  ...
  /** Uniform r-grid of real-space force factors (includes prefactor). */
  std::vector<double> m_sr_force_table;
  double m_sr_dr_inv = 0.;
};
