	...
	.option nopic
	.attribute arch, "rv64i2p1_m2p0_a2p1_f2p2_d2p2_c2p0_v1p0_zicsr2p0_zifencei2p0_zba1p0_zbb1p0_zbc1p0_zve32f1p0_zve32x1p0_zve64d1p0_zve64f1p0_zve64x1p0_zvl128b1p0_zvl256b1p0_zvl32b1p0_zvl64b1p0"
	.attribute unaligned_access, 1
	.attribute stack_align, 16
# GNU C17 (GCC) version 14.3.0 (riscv64-unknown-linux-gnu)
#	compiled by GNU C version 14.3.0, GMP version 6.2.1, MPFR version 4.1.0, MPC version 1.2.1, isl version isl-0.24-GMP

# GGC heuristics: --param ggc-min-expand=100 --param ggc-min-heapsize=131072
# options passed: -mabi=lp64d -mtune=spacemit-x60 -misa-spec=20191213 -mtls-dialect=trad -march=rv64imafdcv_zicsr_zifencei_zba_zbb_zbc_zve32f_zve32x_zve64d_zve64f_zve64x_zvl128b_zvl256b_zvl32b_zvl64b -O3
	.text
	.align	1
...
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:19:   uint64_t s = 0;
	li	a0,0		# <retval>,
	sh3add	a1,a1,a5	#, _15, n, ivtmp.10
.L3:
...
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:28:   for (int i = 0; i < n; i++)
	ble	a3,zero,.L11	#, n,,
.L10:
	vsetvli	a5,a3,e8,mf8,ta,ma	# ivtmp_25, _21,,,,
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:29:     x = a[i] * b[i] + c[i] + x;
	vle64.v	v4,0(a0)	# vect__4.21,* a
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:28:   for (int i = 0; i < n; i++)
	sub	a3,a3,a5	# ivtmp_25, ivtmp_25, _21
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:29:     x = a[i] * b[i] + c[i] + x;
	vle64.v	v2,0(a1)	# vect__6.24,* b
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:28:   for (int i = 0; i < n; i++)
	sh3add	a0,a5,a0	#, a, _21, a
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:29:     x = a[i] * b[i] + c[i] + x;
	vle64.v	v3,0(a2)	# vect__9.28,* c
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:28:   for (int i = 0; i < n; i++)
	sh3add	a1,a5,a1	#, b, _21, b
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:29:     x = a[i] * b[i] + c[i] + x;
	vsetivli	zero,1,e64,m1,ta,ma	#,,,,
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:28:   for (int i = 0; i < n; i++)
	sh3add	a2,a5,a2	#, c, _21, c
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:29:     x = a[i] * b[i] + c[i] + x;
	vfmv.s.f	v1,fa0	# tmp152, <retval>
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:29:     x = a[i] * b[i] + c[i] + x;
	vsetvli	zero,a5,e64,m1,ta,ma	# _21,,,,
	vfmadd.vv	v2,v4,v3	# vect__10.29, vect__4.21, vect__9.28,
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:29:     x = a[i] * b[i] + c[i] + x;
	vfredosum.vs	v1,v2,v1	# tmp153, vect__10.29, tmp152,
	vfmv.f.s	fa0,v1	# <retval>, tmp153
	bne	a3,zero,.L10	#, ivtmp_25,,
	ret	
.L11:
...
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:37:     s += a[i] / (b[i] | 1);
	lw	a5,0(a1)		# _6, MEM[(int32_t *)_39]
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:37:     s += a[i] / (b[i] | 1);
	lw	a4,0(a3)		# _4, MEM[(int32_t *)_40]
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:36:   for (int i = 0; i < n; i++) {
	addi	a3,a3,4	#, ivtmp.39, ivtmp.39
	addi	a1,a1,4	#, ivtmp.40, ivtmp.40
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:37:     s += a[i] / (b[i] | 1);
	ori	a6,a5,1	#, tmp152, _6
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:38:     s ^= a[i] + b[i];
	addw	a5,a4,a5	# _6, tmp159, _4
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:37:     s += a[i] / (b[i] | 1);
	divw	a4,a4,a6	# tmp152, tmp155, _4
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:37:     s += a[i] / (b[i] | 1);
	addw	a0,a4,a0	# <retval>, tmp157, tmp155
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:38:     s ^= a[i] + b[i];
	xor	a0,a0,a5	# tmp159, <retval>, tmp157
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:36:   for (int i = 0; i < n; i++) {
	bne	a2,a3,.L17	#, _35, ivtmp.39,
	ret	
.L18:
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:35:   int32_t s = 1;
	li	a0,1		# <retval>,
...
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:46:   for (int i = 0; i < n; i++)
	ble	a2,zero,.L24	#, n,,
	vsetvli	a5,zero,e64,m1,ta,ma	#, tmp146,,,,
	vmv.v.i	v1,0	# vect_s_19.49,
.L23:
	vsetvli	a5,a2,e64,m1,tu,ma	# ivtmp_9, _8,,,,
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:47:     s += (a[i] << 1) + b[i];
	vle64.v	v3,0(a1)	# vect__7.56,* b
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:46:   for (int i = 0; i < n; i++)
	sub	a2,a2,a5	# ivtmp_9, ivtmp_9, _8
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:47:     s += (a[i] << 1) + b[i];
	vle64.v	v2,0(a0)	# vect__4.52,* a
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:46:   for (int i = 0; i < n; i++)
	sh3add	a1,a5,a1	#, b, _8, b
	sh3add	a0,a5,a0	#, a, _8, a
	vadd.vv	v1,v3,v1	# vect__10.57, vect__7.56, vect_s_19.49,
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:47:     s += (a[i] << 1) + b[i];
	vsll.vi	v2,v2,1	#, vect__5.53_30, vect__4.52,
# /home/orangepi/gcc-tune/measurements/canaries/sched_kernels.c:47:     s += (a[i] << 1) + b[i];
	vadd.vv	v1,v1,v2	# vect_s_19.49, vect__10.57, vect__5.53_30,
	bne	a2,zero,.L23	#, ivtmp_9,,
	vsetvli	a5,zero,e64,m1,ta,ma	#, tmp153,,,,
	li	a4,0		# tmp154,
	vmv.s.x	v2,a4	# tmp152, tmp154
	vredsum.vs	v1,v1,v2	# tmp155, vect_s_19.49, tmp152,
	vmv.x.s	a0,v1	# <retval>, tmp155
	ret	
.L24:
