#!/usr/bin/env bash
set -eo pipefail
OUT=/home/orangepi/gcc-tune/measurements/results/rv2-gcc152-openblas-hpl-ab
ROOT=/home/orangepi/gcc-tune
GCC152=$ROOT/install/gcc-15.2.0-x60/bin/gcc
GCC152_LIB=$ROOT/install/gcc-15.2.0-x60/lib
...
link_one() {
  ...
  "$GCC152" -O2 -B"$AS_BINDIR/" \
    -o "$OUT/bin/bench-$name" "$OUT/bin/bench_dgemm.c" "$a" \
    -L"$GCC152_LIB" -Wl,-rpath,"$GCC152_LIB" \
...
# Modest HPL: rebuild xhpl against each static OpenBLAS if HPL sources exist
...
: > "$HPL_LOG"
{
  ...
  # Try EESSI MPI + rebuild, else skip gracefully
  set +u
  ...
  build_xhpl() {
    ...
    rm -rf "$bdir"
    mkdir -p "$bdir"
    # Use existing Make.UNKNOWN or create minimal
    local arch=Linux_RISCV64_$name
...
LAdir        =
LAinc        =
LAlib        = $lib -lgfortran -lm -lpthread -ldl -fopenmp
F2CDEFS      =
HPL_OPTS     = -DHPL_DETAILED_TIMING -DHPL_PROGRESS_REPORT
HPL_INCLUDES = -I\$(INCdir) -I\$(INCdir)/\$\(ARCH\) \$(LAinc) \$(MPinc)
CC           = $MPICC
CCNOOPT      = \$(HPL_INCLUDES)
CCFLAGS      = \$(HPL_INCLUDES) -O3 -march=rv64gcv_zba_zbb_zbc_zvl256b -mabi=lp64d -fomit-frame-pointer -DADD_
LINKER       = $MPICC
LINKFLAGS    = \$(CCFLAGS) -L$GCC152_LIB -Wl,-rpath,$GCC152_LIB -L$GFORTRAN_LIB -Wl,-rpath,$GFORTRAN_LIB
ARCHIVER     = ar
ARFLAGS      = r
RANLIB       = echo
EOF
    # Fix typo in includes line
    sed -i 's|\\(\$(ARCH)\\)|$(ARCH)|' "$HPL_SRC/Make.$arch" || true
    sed -i 's|\$\\\((ARCH)\\\)|$(ARCH)|g' "$HPL_SRC/Make.$arch" || true
    # rewrite cleanly
    cat > "$HPL_SRC/Make.$arch" <<EOF
SHELL        = /bin/sh
CD           = cd
...
EOF
  ...
  export LD_LIBRARY_PATH="$GFORTRAN_LIB:$GCC152_LIB:${LD_LIBRARY_PATH:-}"
...
out = Path("/home/orangepi/gcc-tune/measurements/results/rv2-gcc152-openblas-hpl-ab")
...
lines = ["# OpenBLAS DGEMM mtune A/B (GCC 15.2-x60)", "",
         "OpenBLAS `RISCV64_ZVL256B`, `NO_SHARED=1`, `USE_OPENMP=1`, built with gcc-15.2.0-x60.",
