#!/usr/bin/env bash
# Deploy m1pack + fp16 MatMulNBits native path, rebuild ORT, benchmark Qwen.
set -euo pipefail
...
log="$LOG_DIR/onnx-matmul-fp16-opt-$stamp.log"
...
echo "=== matmul fp16-native deploy $stamp ==="
bash "$PATCH_DIR/apply-matmul-fp16-opt.sh"

cd "$ORT_BUILD"
rm -f libonnxruntime.so libonnxruntime.so.1.29.0
if command -v ninja >/dev/null 2>&1; then
  ninja -j"$(nproc)" onnxruntime_mlas onnxruntime
else
  make -j"$(nproc)" onnxruntime_mlas onnxruntime
fi

echo "=== verify FromFp16 + M1 symbols ==="
nm libonnxruntime_mlas.a | grep -E "FromFp16|M1Kernel_CompInt8|QuantizeARow" | head -10

echo "=== rebuild + run real LLM runner ==="
