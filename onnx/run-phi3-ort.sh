#!/usr/bin/env bash
# Build + run Phi-3-mini-4k Instruct int4 through m1pack ORT on X60.
set -euo pipefail
...
# Phi-3-mini: 32 layers, MHA 32 KV heads, head_dim 96
export LLM_KV_HEADS="${LLM_KV_HEADS:-32}"
export LLM_HEAD_DIM="${LLM_HEAD_DIM:-96}"
export LLM_LAYERS="${LLM_LAYERS:-32}"
export LLM_MAX_SEQ="${LLM_MAX_SEQ:-256}"
# Chat: user "What is 2+2?" + assistant prompt
export LLM_PROMPT_TOKENS="${LLM_PROMPT_TOKENS:-32010,1724,338,29871,29906,29974,29906,29973,32007,32001}"
export LLM_EOS_IDS="${LLM_EOS_IDS:-32000,32001,32007}"
...
echo "=== run Phi-3 decode (taskset cores for ${THREADS} threads) ==="
