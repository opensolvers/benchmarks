...
if(NOT "${CMAKE_CXX_COMPILER_ID}" STREQUAL "GNU" OR CMAKE_CXX_COMPILER_VERSION VERSION_GREATER "11")
    ...
    if(RISCV64 AND MLAS_SOURCE_IS_NOT_SET)
        ...
        list(APPEND mlas_platform_srcs
          ${MLAS_SRC_DIR}/sconv_nchw_depthwise_multiplier_1.cpp)
        list(APPEND mlas_platform_srcs
          ${MLAS_SRC_DIR}/riscv64/sqnbitgemm_kernel_scalar.cpp)
        ...
        if(onnxruntime_USE_RVV)
          ...
          if(HAS_RISCV64_RVV)
            list(APPEND mlas_platform_srcs
              ...
              ${MLAS_SRC_DIR}/riscv64/sqnbitgemm_kernel_rvv.cpp
            )
            list(REMOVE_ITEM mlas_platform_srcs
              "${MLAS_SRC_DIR}/sconv_nchw_depthwise_multiplier_1.cpp")
            ...
            # SpaceMiT X60 IME (smt.vmadot) block-quantized (SQNBit) GEMM kernel.
            # Needs an assembler carrying the xsmtvdot vendor extension (binutils
            # 2.46.1 + add-spacemit-xsmtvdot, e.g. via EESSI); probe for it. Runtime
            # use is additionally gated on X60 detection in platform.cpp.
            set(OLD_CMAKE_REQUIRED_FLAGS "${CMAKE_REQUIRED_FLAGS}")
            set(CMAKE_REQUIRED_FLAGS "${OLD_CMAKE_REQUIRED_FLAGS} -march=rv64gcv -mabi=lp64d")
            check_cxx_source_compiles("
              int main() { __asm__(\".option arch, +xsmtvdot\"); __asm__(\"smt.vmadot v16, v8, v9\"); return 0; }"
              HAS_RISCV64_IME
            )
            set(CMAKE_REQUIRED_FLAGS "${OLD_CMAKE_REQUIRED_FLAGS}")
            unset(OLD_CMAKE_REQUIRED_FLAGS)
            if(HAS_RISCV64_IME)
              list(APPEND mlas_platform_srcs
                ${MLAS_SRC_DIR}/riscv64/sqnbitgemm_kernel_ime.cpp)
              set_source_files_properties(
                ${MLAS_SRC_DIR}/riscv64/sqnbitgemm_kernel_ime.cpp
                PROPERTIES COMPILE_FLAGS "-march=rv64gcv -mabi=lp64d")
              list(APPEND mlas_private_compile_definitions MLAS_USE_IME=1)
            endif()

            if(onnxruntime_USE_RVV_ZVFH)
              list(APPEND mlas_platform_srcs
                ${MLAS_SRC_DIR}/riscv64/halfgemm_kernel_rvv.cpp
                ${MLAS_SRC_DIR}/riscv64/hgemm_kernel_rvv.cpp
                ${MLAS_SRC_DIR}/riscv64/softmax_kernel_rvv_fp16.cpp
                ${MLAS_SRC_DIR}/riscv64/eltwise_kernel_rvv.cpp
