...
class GQAAttentionBase {
 ...
 private:
  ...
  void ComputeAttentionProbs(U* attention_probs,                                   // output probs [B, N, S, T]
    ...
    ThreadPool::TryParallelFor(tp, loop_len, unit_cost, [&](std::ptrdiff_t begin, std::ptrdiff_t end) {
      for (std::ptrdiff_t i = begin; i != end; ++i) {
        ...
        } else if constexpr (std::is_same<T, MLFloat16>::value && std::is_same<U, float>::value) {
          if (MlasHGemmSupported(CblasNoTrans, CblasTrans)) {
            const size_t score_count = SafeInt<size_t>(sequence_length) * present_buffer_sequence_length;
            InlinedVector<MLFloat16> scores_fp16(score_count);
            MlasGemm(CblasNoTrans, CblasTrans, static_cast<int>(sequence_length), static_cast<int>(total_seqlen),
                     static_cast<int>(head_size), q, static_cast<int>(head_size), k, static_cast<int>(head_size),
                     scores_fp16.data(), static_cast<int>(present_buffer_sequence_length),
                     MLFloat16(alpha).val, static_cast<uint16_t>(0), nullptr);
            MlasConvertHalfToFloatBuffer(scores_fp16.data(), output, score_count);
          } else {
  ...
  void ComputeVxAttentionScore(T* output,                                    // buffer for the result with size BxSxNxH
    ...
    bool fp16_hgemm_v_path = false;
    if constexpr (std::is_same<T, MLFloat16>::value && std::is_same<U, float>::value) {
      fp16_hgemm_v_path = MlasHGemmSupported(CblasNoTrans, CblasNoTrans);
      if (!fp16_hgemm_v_path) {
    ...
    ThreadPool::TryParallelFor(tp, loop_len, unit_cost, [&](std::ptrdiff_t begin, std::ptrdiff_t end) {
      for (std::ptrdiff_t i = begin; i != end; ++i) {
        ...
        if constexpr (std::is_same<T, float>::value) {
          ...
          if (fp16_hgemm_v_path) {
            const size_t prob_count = SafeInt<size_t>(sequence_length) * total_seqlen;
            InlinedVector<MLFloat16> probs_fp16(prob_count);
            MlasConvertFloatToHalfBuffer(attention_probs + attention_probs_offset, probs_fp16.data(), prob_count);
            T* output_current = output + (batch_index * sequence_length * num_heads_ + head_index) * head_size;
            MlasGemm(CblasNoTrans, CblasNoTrans, static_cast<int>(sequence_length), static_cast<int>(head_size),
                     static_cast<int>(total_seqlen), probs_fp16.data(),
                     static_cast<int>(present_buffer_sequence_length), v, static_cast<int>(head_size),
                     output_current, static_cast<int>(hidden_size), MLFloat16(1.0f).val,
                     static_cast<uint16_t>(0), nullptr);
          } else {
