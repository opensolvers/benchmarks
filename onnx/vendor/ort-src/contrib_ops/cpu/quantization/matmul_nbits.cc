...
Status MatMulNBits<MLFloat16>::ComputeBPacked(const Tensor* a,
  ...
  IAllocatorUniquePtr<float> tmp_scales;
  if (!scales_fp32_) {
    ORT_ENFORCE(scales != nullptr, "scales must be provided when not pre-converted");
    tmp_scales = IAllocator::MakeUniquePtr<float>(allocator, static_cast<size_t>(scales->Shape().Size()), true);
    MlasConvertHalfToFloatBuffer(scales_data, tmp_scales.get(), static_cast<size_t>(scales->Shape().Size()));
    scales_ptr = tmp_scales.get();
  } else {
    scales_ptr = scales_fp32_.get();
  }
  ...
  if (bias_data) {
    if (!bias_fp32_) {
      tmp_bias = IAllocator::MakeUniquePtr<float>(allocator, static_cast<size_t>(bias->Shape().Size()), true);
      MlasConvertHalfToFloatBuffer(bias_data, tmp_bias.get(), static_cast<size_t>(bias->Shape().Size()));
      bias_ptr = tmp_bias.get();
    } else {
      bias_ptr = bias_fp32_.get();
    }
  ...
  if (compute_type_ == SQNBIT_CompInt8 && MlasIsQNBitGemmCompInt8FromFp16Available()) {
    InlinedVector<MLAS_QNBIT_GEMM_COMPINT8_FP16A_PARAMS> data(batch_count);
    for (size_t i = 0; i < batch_count; ++i) {
      data[i].A = a_data + helper.LeftOffsets()[i];
      data[i].lda = lda;
      data[i].QuantBDataWorkspace = packed_b_.get();
    ...
    MlasQNBitGemmBatch_CompInt8_FromFp16A(M, N, K, batch_count, nbits_, block_size_, data.data(),
                                          workspace.get(), thread_pool, &mlas_backend_kernel_selector_config_);
    MlasConvertFloatToHalfBuffer(tmp_c.get(), y_data, c_size);
    return Status::OK();
  }
