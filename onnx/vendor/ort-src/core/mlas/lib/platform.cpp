...
--*/
{
    ...
    // Portable scalar SQNBit (block-quantized 4-bit) GEMM: the riscv64 base
    // fallback, overridden by the RVV then IME backends below when available.
    this->QNBitGemmDispatch = &GetMlasQNBitGemmDispatchScalar();
    ...
    this->QNBitGemmDispatch = &GetMlasQNBitGemmDispatchRvv();
...
#if defined(MLAS_USE_IME)
    // SpaceMiT X60 IME (smt.vmadot) block-quantized SQNBit CompInt8 GEMM. Compiled
    // only where the assembler carries xsmtvdot; smt.vmadot faults on non-IME
    // hardware, so a runtime X60 uarch check should gate this before enabling on
    // generic riscv64 (build-time guard only for now).
    this->QNBitGemmDispatch = &GetMlasQNBitGemmDispatchIme();
#endif

#if defined(MLAS_USE_RVV_ZVFH)
    // X60 kernels are built with -march=..._zvfh; Linux hwprobe often omits
    // RISCV_HWPROBE_EXT_ZVFH on current boards, so wire fp16 dispatches when
    // compiled in rather than relying on HasFp16VectorAcceleration().
    this->CastF16ToF32Kernel = &MlasCastF16ToF32KernelRvv;
    this->CastF32ToF16Kernel = &MlasCastF32ToF16KernelRvv;
    this->HGemmDispatch = &MlasHGemmDispatchRvv;
    this->SoftmaxDispatch = &MlasSoftmaxDispatchRvv;
    this->EltwiseDispatch = &MlasEltwiseDispatchRvv;
#endif
